...
func (m model) handleLeftClick(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	...
	// Skip if clicking on title or status bar
	if m.isInTitleBar(x, y) || m.isInStatusBar(x, y) {
		return m, nil
	}

	// Calculate content area boundaries
	titleHeight := 3 // title + instructions + separator
	statusHeight := 1
	...
	// Detect panel clicks based on current mode
	if m.showThreePanel {
		// 3-panel mode: top left/right, bottom
		leftWidth, rightWidth, topHeight, _ := m.calculateThreePanelLayout()

		relY := y - contentStartY

		if relY < topHeight {
			// Click in top panels (left or right)
			if x < leftWidth {
				m.focusedPanel = "left"
				m.statusMsg = "Focused LEFT panel (clicked)"
			} else if x > leftWidth && x < leftWidth+1+rightWidth {
				m.focusedPanel = "right"
				m.statusMsg = "Focused RIGHT panel (clicked)"
			}
		} else if relY > topHeight {
			// Click in bottom panel (skip horizontal divider at relY == topHeight)
			m.focusedPanel = "bottom"
			m.statusMsg = "Focused BOTTOM panel (clicked)"
		}
	} else if m.shouldUseVerticalStack() {
		// Vertical stack mode: panels stacked top to bottom
		topHeight, _ := m.calculateVerticalStackLayout()

		relY := y - contentStartY

		if relY < topHeight {
			// Click in top panel (left content)
			m.focusedPanel = "left"
			m.statusMsg = "Focused LEFT panel (clicked)"
		} else if relY > topHeight {
			// Click in bottom panel (right content)
			// Skip horizontal divider at relY == topHeight
			m.focusedPanel = "right"
			m.statusMsg = "Focused RIGHT panel (clicked)"
		}
	} else {
		// 2-panel mode: left and right side-by-side
		leftWidth, _ := m.calculateDualPaneLayout()

		if x < leftWidth {
			m.focusedPanel = "left"
			m.statusMsg = "Focused LEFT panel (clicked)"
		} else if x > leftWidth { // Skip divider at x == leftWidth
			m.focusedPanel = "right"
			m.statusMsg = "Focused RIGHT panel (clicked)"
		}
	}
...
func (m model) handleMouseMotion(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	...
	// Detect which panel is being hovered
	titleHeight := 3
	statusHeight := 1
	contentStartY := titleHeight
	contentEndY := m.height - statusHeight
	...
	if msg.Y >= contentStartY && msg.Y < contentEndY {
		if m.showThreePanel {
			...
			if relY < topHeight {
				if msg.X < leftWidth {
					m.hoveredItem = "left-panel"
				} else if msg.X > leftWidth && msg.X < leftWidth+1+rightWidth {
					m.hoveredItem = "right-panel"
				}
			} else if relY > topHeight {
				m.hoveredItem = "bottom-panel"
			}
		} else if m.shouldUseVerticalStack() {
			// Vertical stack mode: detect top vs bottom panel by Y position
			topHeight, _ := m.calculateVerticalStackLayout()
			relY := msg.Y - contentStartY
			...
			// Side-by-side mode: detect left vs right panel by X position
			leftWidth, _ := m.calculateDualPaneLayout()
			if msg.X < leftWidth {
				m.hoveredItem = "left-panel"
